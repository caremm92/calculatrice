...
boolean correct=true;
   ...
   if(!Number.isNumber(scan.get(index-1))&&!isBinaryOperator(scan.get(index-1))&&
           !Variable.isVariableString(scan.get(index-1))&&!isBracket(scan.get(index-1))
           ){
             Utils.logError(
       ...
       //specify valid tokens that can come after a LogOrAntiLogToAnyBase operator
         if(!Number.isNumber(scan.get(index+1))&&!isOpeningBracket(scan.get(index+1))&&!Method.isNumberReturningStatsMethod(scan.get(index+1))
