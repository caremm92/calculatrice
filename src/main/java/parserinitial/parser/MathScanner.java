...
import parserinitial.parser.methods.Method;

import parserinitial.math.numericalmethods.RootFinder;
import java.util.*;
import java.util.ArrayList;

import parserinitial.math.numericalmethods.NumericalIntegral;
import parserinitial.util.*;


import parserinitial.math.differentialcalculus.Parser;
    ...
    public MathScanner(String scannerInput) {
            ...
            try {
                if (scannerInput.substring(i, i + 1).equals("π")) {
                    ...
                    scannerInput = STRING.replace(scannerInput, Operator.DIVIDE, i, i + 1);
                        ...
                        && STRING.isDigit(scannerInput.substring(i + 2, i + 3))) {
                    ...
                    if (scannerInput.substring(i + 1, i + 2).equals(Operator.PLUS)) {
                        ...
                        scannerInput = STRING.replace(scannerInput, Operator.EN_DASH, i + 1, i + 2);//replacing the minus operator with the En-Dash symbol
                    }
                }
        ...
        for (int i = 0; i < scannerInput.length(); i++) {
            if (scannerInput.substring(i, i + 1).equals(",")
                    ...
                    && (scannerInput.substring(i + 2, i + 3).equals(".") || STRING.isDigit(scannerInput.substring(i + 2, i + 3)))) {
                scannerInput = STRING.replace(scannerInput, "~", i + 1, i + 2);
            }
        ...
        if (num2.equals(".")) {
            try {
                for (int i = ind; i < val.length(); i++) {
                    if (!STRING.isNumberComponent(val.substring(i, i + 1))) {
                        break;
                    }
                    ...
                    if (STRING.isDigit(val.substring(i, i + 1)) && ((pointCount == 0 && expCount == 1) || (pointCount == 1 && expCount == 1))) {
...
//klnsdkn23.94E2jwjji
    ...
    public ArrayList<String> splitStringAtFirstNumber(String val) {
        int firstOccDigit = STRING.getFirstIndexOfDigitOrPoint(val);//record the index where a digit or point first occurs.
...
//CASE 1: i.e a number starts the input string.
    ...
    public ArrayList<String> splitStringOnNumbers(String val) {
        ...
        boolean canSplit = STRING.getFirstIndexOfDigitOrPoint(val) > -1;
        if (canSplit) {
            int passes = 0;
            while (canSplit) {
                split.clear();
                ...
                canSplit = STRING.getFirstIndexOfDigitOrPoint(val) > -1;
...
//
    ...
    public void splitStringOnMethods_Variables_And_Operators() {
        CustomScanner cs = new CustomScanner(scannerInput, true, VariableManager.VARIABLES.keySet().toArray(new String[]{}), Method.getAllFunctions(), Operator.operators);
        ...
        for (int i = 0; i < scanner.size(); i++) {
            ...
            if (Number.isNumber(token) && !Number.validNumber(token)) {
                ...
                for (int j = 0; j < token.length(); j++) {//3.4567A
                    if (!STRING.isNumberComponent(token.substring(j, j + 1)) && Variable.isVariableString(token.substring(j))) {
            ...
            if (i+1<scanner.size() && (Number.validNumber(token) || Variable.isVariableString(token)) && Variable.isVariableString(scanner.get(i + 1))) {
                scanner.add(i + 1, "*");
            }
            /**
             * Allow early conversion of )sin(k) into )*sin(k)
             */
            else if (i+1<scanner.size() && Operator.isClosingBracket(token) && Variable.isVariableString(scanner.get(i + 1)) &&
                ...
                if(open>0 && !Operator.isAtOperator(scanner.get(open-1)) && !Method.isListReturningStatsMethod(scanner.get(open-1))){
                    scanner.add(i + 1, "*");
                }
            }
            else if(i+1<scanner.size() && Number.isNumber(token)&& Operator.isOpeningBracket(scanner.get(i+1))){
                scanner.add(i+1,"*");
            }
        ...
        for (int i = 0; i < scanner.size(); i++) {
            try {
                if (Method.isMethodName(scanner.get(i)) && (Number.isNumber(scanner.get(i + 1)) || Method.isMethodName(scanner.get(i + 1)))) {
...
//sort(5,3,2,1,-8,-9,12,34,98,-900,34,23,12,340)
        ...
        for (int i = 0; i < scanner.size(); i++) {//±–
            if (scanner.get(i).contains("±")) {
                ...
                scanner.set(i, STRING.replace(scanner.get(i), "+", index, index + 1));
            } else if (scanner.get(i).contains("–")) {//replace the en-dash symbol with the minus (-) symbol again..
                int index = scanner.get(i).indexOf("–");
                scanner.set(i, STRING.replace(scanner.get(i), "-", index, index + 1));
            }
            if (Operator.isOpeningBracket(scanner.get(i)) && scanner.get(i + 1).equals("-") && Number.isNumber(scanner.get(i + 2))) {

                int index = scanner.get(i+2).indexOf(Operator.EN_DASH);
                if (index != -1) {//In case the number at i+2 contains an EN_DASH, replace it with a minus
                    scanner.set(i+2, STRING.replace(scanner.get(i+2), Operator.MINUS, index, index + 1));
                }
    ...
    public void validateInputAfterSplitOnMethodsAndOps() {
        for (int i = 0; i < scanner.size(); i++) {
            try {
                 ...
                 */
                if ((Method.isFunctionOperatingMethod(scanner.get(i)) || Method.isStatsMethod(scanner.get(i))) && Operator.isOpeningBracket(scanner.get(i + 1))) {
                 ...
                 */
                else if ((Number.isNumber(scanner.get(i)) || (Variable.isVariableString(scanner.get(i)) && !Method.isDefinedMethod(scanner.get(i))))
                 ...
                 */
                else if (Number.isNumber(scanner.get(i)) && Method.isDefinedMethod(scanner.get(i + 1))) {
                    scanner.add(i + 1, "*");
                    i++;
                }//end if
                /**
                 ...
                 */
                else if (Operator.isClosingBracket(scanner.get(i)) && (Variable.isVariableString(scanner.get(i + 1)) || Number.isNumber(scanner.get(i + 1)))) {
                    scanner.add(i + 1, "*");
                    i++;
                }//end else if
                /**
                 ...
                 */
                else if (Operator.isClosingBracket(scanner.get(i)) && Operator.isOpeningBracket(scanner.get(i + 1))) {
                    scanner.add(i + 1, "*");
                    i++;
                }
    ...
    public void validateTokens() {
        for (int i = 0; i < scanner.size(); i++) {
            try {
                ...
                if (!Operator.isOperatorString(scanner.get(i)) && !Variable.isVariableString(scanner.get(i))
                        && !Number.validNumber(scanner.get(i)) && !Method.isMethodName(scanner.get(i))) {
...
//enable interpretation of things like 3^-4 or 3^+4 i.e ^- or ^+ patterns
        for (int i = 0; i < scanner.size(); i++) {
            try {
                if (Operator.isPower(scanner.get(i))) {
                    if (scanner.get(i + 1).equals("-") && Number.validNumber(scanner.get(i + 2))) {
                        scanner.set(i + 1, String.valueOf(-1 * Double.parseDouble(scanner.get(i + 2))));
                        scanner.remove(i + 2);
                    } else if (scanner.get(i + 1).equals("+") && Number.validNumber(scanner.get(i + 2))) {
        ...
        for (int i = 0; i < scanner.size(); i++) {
             ...
             */
            if (i+1<sz && Method.isFunctionOperatingMethod(scanner.get(i)) && Operator.isOpeningBracket(scanner.get(i + 1))) {
                int close = Bracket.getComplementIndex(true, i + 1, scanner);
                i = close;
                continue;
            }//end if

            if (!Variable.isVariableString(scanner.get(i)) && !Operator.isOperatorString(scanner.get(i)) && !Number.validNumber(scanner.get(i))
...
//enable interpretation of things like 3^-4 or 3^+4 i.e ^- or ^+ patterns
        ...
        for (int i = 0; i < scanner.size(); i++) {
            ...
            if (MathExpression.isAutoInitOn()) {
                if (i+1<sz && Variable.isVariableString(scanner.get(i)) && !Operator.isOpeningBracket(scanner.get(i + 1)) && !varMan.contains(scanner.get(i))
        ...
        while ((indexOfAt = scanner.indexOf("@")) != -1) {
            ...
            if (Operator.isOpeningBracket(scanner.get(indexOfAt + 1))) {
                ...
                for (int i = indexOfAt; i < scanner.size(); i++) {
                    ...
                    if (Operator.isOpeningBracket(token)) {
                    ...
                    } else if (Operator.isComma(token)) {
                        ...
                        break;
                    } else if (Operator.isClosingBracket(token)) {
                        int open = Bracket.getComplementIndex(false, i, scanner);
    ...
    public static void $removeExcessBrackets(List<String> scanner) {
        ...
        for (int i = 0; i < scanner.size(); i++) {
            ...
            if (i + 1 < scanner.size() && Operator.isClosingBracket(scanner.get(i))) {

                if (Operator.isClosingBracket(scanner.get(i + 1))) {//you have a match for an unnecessary wasted bracket
    ...
    public static void removeExcessBrackets(List<String> scanner) {
        ...
        for (int i = 0; i < scanner.size(); i++) {
            ...
            if(Operator.isClosingBracket(scanner.get(i))){
                ...
                if(i+1<scanner.size() && Operator.isOpeningBracket(scanner.get(i+1)) ){//if the pattern is )(...ignore the closing bracket.
                    continue;
                }
    ...
    public static void extractFunctionStringFromExpressionForMatrixMethods(List<String> list){
        ...
        if(list.indexOf("(") == list.lastIndexOf("(") && list.indexOf(")") == list.lastIndexOf(")")){
            ...
            if(sz == 4 || sz == 5 ){
                if(Method.isMatrixMethod(list.get(0)) && Operator.isOpeningBracket(list.get(1)) && Method.isUserDefinedFunction(list.get(2))  ){
                    if(sz==4 && Operator.isClosingBracket(list.get(3))){
                        Method.run(list, 1);
                    }
                    else if(sz==5 && (Method.isUserDefinedFunction(list.get(3))|| Number.isNumber(list.get(3))|| Variable.isVariableString(list.get(3))) && Operator.isClosingBracket(list.get(4))){System.out.println("Debug--4");
                        Method.run(list, 1);
                    }
            ...
            for(int i=0; i<list.size();i++){
                ...
                if( Operator.isClosingBracket(list.get(i)) ){
                    ...
                    if(open>0){
                        ...
                        else if(Method.isMethodName(token)|| Operator.isUnaryPreOperator(token)|| Number.isNumber(token)){
