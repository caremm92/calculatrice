    ...
    boolean correct = true;
        try{
         ...
         if(!isNumber(scan.get(index-1))&&!isClosingBracket(scan.get(index-1))&&
          !Variable.isVariableString(scan.get(index-1))&&!isUnaryPostOperator(scan.get(index-1))
          ){
           Utils.logError(
    ...
    public static void assignCompoundTokens(ArrayList<String> scan) {
        ...
        for (int i = 0; i < scan.size(); i++) {
            if (isUnaryPostOperator(scan.get(i))) {
                ...
                if (isNumber(scan.get(i - 1)) || Variable.isVariableString(scan.get(i - 1))) { //e.g. change: 9² to (9²) and 9²² to (9²²) etc.
                    int index = i - 1;
                    int j = i;
