...
import parserinitial.interfaces.Savable;
import parserinitial.parser.methods.Method;

import parserinitial.math.Maths;
...
import parserinitial.math.matrix.expressParser.Matrix;
...
//if DRG==1, it is done in radians and if it is 2, it is done in grads.
    ...
    public ArrayList<Variable> getVars() {
        ...
        for (int i = 0; i < scanner.size(); i++) {
            if (Variable.isVariableString(scanner.get(i)) && !Operator.isOpeningBracket(scanner.get(i + 1))) {
    ...
    public void unBracketDataSetReturningStatsOperators() {
        ...
        for (int i = 0; i < scanner.size(); i++) {
            ...
            if (Method.isListReturningStatsMethod(scanner.get(i))) {
                 ...
                 *
                 */
                while (Operator.isOpeningBracket(scanner.get(I))) {
                    ...
                    } else if (I == 0 && Operator.isOpeningBracket(scanner.get(0))) {
                        scanner.set(closeBracIndex, "");
                        scanner.set(0, "");
                        exitForLoop = closeBracIndex <= scanner.size();
                        break;
    ...
    public void statsVerifier() {
        ...
        for (int i = 0; i < scanner.size(); i++) {
            if (Method.isListReturningStatsMethod(scanner.get(i)) && Operator.isOpeningBracket(scanner.get(i + 1))) {
        ...
        if (noOfListReturningOperators > 0 && correctFunction) {
            ...
            if (isHasListReturningOperators()) {
                ...
                for (int i = 0; i < scanner.size(); i++) {
                    try {
                        if (Method.isListReturningStatsMethod(scanner.get(i)) && Operator.isOpeningBracket(scanner.get(i + 1))) {
                            if (Operator.isBinaryOperator(scanner.get(i - 1))) {
                                //processLogger.writeLog("Invalid Association Discovered For: \""+scanner.get(i-1)+"\" And \""+scanner.get(i)+"\".\n");
                                correctFunction = false;
                                break;
                            }
                            if (Operator.isBracket(scanner.get(i - 1)) && !Method.isNumberReturningStatsMethod(scanner.get(i - 2))
    ...
    public void codeModifier() {
        ...
        if (correctFunction) {
            ...
            for (int i = 0; i < scanner.size(); i++) {
                try {
                    if ((Operator.isBinaryOperator(scanner.get(i)) || Method.isUnaryPreOperatorORDefinedMethod(scanner.get(i))
                            || Operator.isOpeningBracket(scanner.get(i))
                            || Operator.isLogicOperator(scanner.get(i)) || Operator.isAssignmentOperator(scanner.get(i))
                            || Operator.isComma(scanner.get(i)) || Method.isStatsMethod(scanner.get(i)))
                            && Operator.isPlusOrMinus(scanner.get(i + 1)) && Number.isNumber(scanner.get(i + 2))) {
            ...
            for (int i = 0; i < scanner.size(); i++) {
                 ...
                 */
                try {
                    if ((Number.isNumber(scanner.get(i)) || (Variable.isVariableString(scanner.get(i)) && !Method.isDefinedMethod(scanner.get(i)))) && (Method.isUnaryPreOperatorORDefinedMethod(scanner.get(i + 1))
                        ...
                        if (Number.isNumber(scanner.get(j)) || Variable.isVariableString(scanner.get(j))) {
                            scanner.add(j + 1, ")");
                            scanner.add(i, "(");
                            scanner.add(i + 2, Operator.MULTIPLY);
                        } else if (Operator.isOpeningBracket(scanner.get(j))) {
                ...
                try {
                     ...
                     */
                    if (Method.isUnaryPreOperatorORDefinedMethod(scanner.get(i - 1)) && Operator.isPlusOrMinus(scanner.get(i)) && Operator.isOpeningBracket(scanner.get(i + 1))) {
                        if (scanner.get(i).equals(Operator.MINUS)) {
                        ...
                        else if (scanner.get(i).equals(Operator.PLUS)) {
                            scanner.set(i, "");
                        }
                ...
                try {
                     ...
                     * -(sin2+3)
                     */
                    if (Operator.isOpeningBracket(scanner.get(i - 1)) && Operator.isPlusOrMinus(scanner.get(i)) && Method.isUnaryPreOperatorORDefinedMethod(scanner.get(i + 1))) {
                        if (scanner.get(i).equals(Operator.MINUS)) {
                            scanner.set(i, "-1");
                            scanner.add(i + 1, Operator.MULTIPLY);
                        } else if (scanner.get(i).equals(Operator.PLUS)) {
                            scanner.set(i, "");
                        }
                    }
    ...
    public void detectKeyOperators() {
        for (int i = 0; i < scanner.size(); i++) {
            if (Operator.isPlusOrMinus(scanner.get(i))) {
                setHasPlusOrMinusOperators(true);
            } else if (Operator.isUnaryPreOperator(scanner.get(i))) {
                setHasPreNumberOperators(true);
            } else if (Operator.isUnaryPostOperator(scanner.get(i))) {
                setHasPostNumberOperators(true);
            } else if (Operator.isMulOrDiv(scanner.get(i))) {
                setHasMulOrDivOperators(true);
            } else if (Operator.isPermOrComb(scanner.get(i))) {
                setHasPermOrCombOperators(true);
            } else if (Operator.isRemainder(scanner.get(i))) {
            ...
            } else if (Operator.isPower(scanner.get(i))) {
                setHasPowerOperators(true);
            } else if (Operator.isLogicOperator(scanner.get(i))) {
...
//check for the various valid arrangements for all members of the function.
                ...
                //Variables
                if (Variable.isVariableString(scanner.get(i)) && !Method.isUserDefinedFunction(token)) {
                    try {
                        //specify valid tokens that can come before a variable
                        if (!Operator.isOpeningBracket(scanner.get(i - 1))
                                && !Operator.isLogicOperator(scanner.get(i - 1)) && !Operator.isUnaryPreOperator(scanner.get(i - 1))
                                && !Operator.isBinaryOperator(scanner.get(i - 1)) && !Operator.isAssignmentOperator(scanner.get(i - 1)) && !Number.isNumber(scanner.get(i - 1))
                                && !Variable.isVariableString(scanner.get(i - 1))) {
                        ...
                        if (!Operator.isBracket(scanner.get(i + 1)) && !Operator.isBinaryOperator(scanner.get(i + 1))
                                && !Operator.isUnaryPostOperator(scanner.get(i + 1)) && !Method.isNumberReturningStatsMethod(scanner.get(i + 1))
                                && !Operator.isLogicOperator(scanner.get(i + 1)) && !Operator.isAssignmentOperator(scanner.get(i + 1))
                                && !Operator.isUnaryPreOperator(scanner.get(i + 1)) && !Method.isNumberReturningStatsMethod(scanner.get(i + 1))
                                && !Method.isLogToAnyBase(scanner.get(i + 1)) && !Method.isAntiLogToAnyBase(scanner.get(i + 1)) && !Number.isNumber(scanner.get(i + 1))
                                && !Variable.isVariableString(scanner.get(i + 1))) {
            ...
            if (correctFunction) {
                setCorrectFunction(Operator.validateAll(scanner));
            }
        ...
        for (int i = 0; i < sz; i++) {
            ...
            try {
                if (i + 1 < sz) {//a next token exists (at i+1) so check the next token
                    if (Variable.isVariableString(scan.get(i)) && !Operator.isOpeningBracket(scan.get(i + 1))) {
    ...
    public String solve() {
        ...
        if (correctFunction && !hasFunctionOrVariableInitStatement) {
            ...
            if (listAppender.startsWith("(")) {
                ...
                if (Operator.isComma(listAppender.substring(0, 1))) {
...
//correct the anomaly: [ (,-,number....,)  ]
        ...
        if (list.get(0).equals("(") && list.get(1).equals(Operator.MINUS) && Number.isNumber(list.get(2))) {
            list.set(1, "");
            ...
            for (int i = 0; i < list.size(); i++) {
                try {
                    if (!list.get(i - 1).equals("Infinity") && !list.get(i + 1).equals("Infinity")) {
                        if (list.get(i).equals(Operator.POWER) && Number.isNumber(list.get(i - 1)) && Number.isNumber(list.get(i + 1))) {
            ...
            for (int i = list.size() - 1; i >= 0; i--) {
                try {
                    if (!list.get(i + 1).equals("Infinity") && Number.isNumber(list.get(i + 1))) {
                        if (list.get(i).equals(Operator.ROOT)) {
                            list.set(i, String.valueOf(Math.sqrt(Double.valueOf(list.get(i + 1)))));
                            list.set(i + 1, "");
                        }//end if
                        if (list.get(i).equals(Operator.CUBE_ROOT)) {
        ...
        if (isHasMulOrDivOperators() || isHasRemainderOperators() || isHasLogicOperators()) {
            for (int i = 0; i < list.size(); i++) {
                ...
                try {
                    ...
                    else if (list.get(i).equals(Operator.REMAINDER)) {
                        if (!list.get(i - 1).equals("Infinity") && !list.get(i + 1).equals("Infinity")) {
                            list.set(i + 1, String.valueOf(Double.valueOf(list.get(i - 1)) % Double.valueOf(list.get(i + 1))));
                            list.set(i - 1, "");
                            list.set(i, "");
                ...
                if (!skip) {
                    try {
                        if (list.get(i).equals(Operator.EQUALS)) {
                    ...
                    try {
                        if (list.get(i).equals(Operator.GREATER_THAN)) {
                    ...
                    try {
                        if (list.get(i).equals(Operator.GREATER_OR_EQUALS)) {
                    ...
                    try {
                        if (list.get(i).equals(Operator.LESS_THAN)) {
                    ...
                    try {
                        if (list.get(i).equals(Operator.LESS_OR_EQUALS)) {
                            if (!list.get(i - 1).equals("Infinity") && !list.get(i + 1).equals("Infinity")) {
            ...
            for (int i = 0; i < list.size(); i++) {
                try {
                    if (list.get(i).equals(Operator.PLUS) || list.get(i).equals(Operator.MINUS)) {
                        if (!list.get(i - 1).equals("Infinity") && !list.get(i + 1).equals("Infinity")) {
                            if (list.get(i).equals(Operator.PLUS) && Number.isNumber(list.get(i - 1)) && Number.isNumber(list.get(i + 1))) {
                                list.set(i + 1, String.valueOf(Double.valueOf(list.get(i - 1)) + Double.valueOf(list.get(i + 1))));
                                list.set(i - 1, "");
                                list.set(i, "");
                            }//end else
                            else if (list.get(i).equals(Operator.MINUS) && Number.isNumber(list.get(i - 1)) && Number.isNumber(list.get(i + 1))) {
                                list.set(i + 1, String.valueOf(Double.valueOf(list.get(i - 1)) - Double.valueOf(list.get(i + 1))));
                                list.set(i - 1, "");
                                list.set(i, "");
                            }//end else if
