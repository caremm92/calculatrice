...
package parserinitial.math.matrix.expressParser;


import parserinitial.math.matrix.*;
import parserinitial.parser.Number;
...
import static parserinitial.parser.STRING.*;
...
for(int i=0;i<scanner.size();i++){
    ...
    if(MatrixVariable.isMatrixVariableName(scanner.get(i))&&!matrixvarman.contains(scanner.get(i))){
        errorList.add(" Unknown Variable: "+scanner.get(i)+ "\n Please Declare And Initialize This Variable Before Using It.\n" +
                "Use The Command, \'var=val\' or\'vars:var1,var2...=val1,val2,... To Accomplish This.");
        setRunnable(false);
    }

    if(!MatrixVariable.isMatrixVariableName(scanner.get(i))&&!Variable.isVariableString(scanner.get(i))&&!MOperator.isOperatorString(scanner.get(i))&&!Number.isNumber(scanner.get(i))){
