 ...
 */
package parserinitial.math.differentialcalculus;

import parserinitial.parser.Bracket;
import parserinitial.parser.Operator;
import java.util.ArrayList;
import static parserinitial.parser.Number.*;
import static parserinitial.parser.Variable.*;
import static parserinitial.parser.Operator.*;
import static parserinitial.parser.Bracket.*;

import parserinitial.parser.PowerOperator;
import parserinitial.parser.UnaryPostOperator;
import parserinitial.parser.methods.*;
...
//check for the various valid arrangements for all members of the function.
            ...
            if (isVariableString(scanner.get(i))) {
                try {
                    ...
                    if (!isOpeningBracket(scanner.get(i - 1))
                            && !isLogicOperator(scanner.get(i - 1)) && !Method.isUnaryPreOperatorORDefinedMethod(scanner.get(i - 1))
                    ...
                    if (!isBracket(scanner.get(i + 1)) && !isBinaryOperator(scanner.get(i + 1))
                            ...
                            && !Method.isLogToAnyBase(scanner.get(i + 1)) && !Method.isAntiLogToAnyBase(scanner.get(i + 1))
    ...
    private void codeModifier() {
        ...
        for (int i = 0; i < scanner.size(); i++) {
             ...
             */
            try {
                if ((isNumber(scanner.get(i)) || isVariableString(scanner.get(i))) && (Method.isUnaryPreOperatorORDefinedMethod(scanner.get(i + 1))
