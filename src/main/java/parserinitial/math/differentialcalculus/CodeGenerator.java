...
import parserinitial.parser.STRING;
 ...
 */ 
    for(int i=0;i<scan.size();i++){
        try{
            ...
            if(Method.isMethodName(scan.get(i))&&isOpeningBracket(scan.get(i+1))){
...
private static void unbracketNumbers(ArrayList<String>scan){
  ////print("Before unbracketNumbers...scan = "+scan);
    for(int i=0;i<scan.size();i++){  
        try{
        if(isNumber(scan.get(i))&&(isOpeningBracket(scan.get(i-1))&&!Method.isMethodName(scan.get(i-2)) )&&isClosingBracket(scan.get(i+1))){
            scan.remove(i+1);
            scan.remove(i-1);
            }
        }//end try
   ...
   if(isPower(scan.get(i)) && isNumber(scan.get(i-1)) && isNumber(scan.get(i+1)) ){
       ...
       //Open the brackets if not belonging to a function.
             if(isOpeningBracket(scan.get(i-2))&&isClosingBracket(scan.get(i+2))){
                 if(i-3>=0 && !Method.isMethodName(scan.get(i-3))){
                 scan.set(i-2,"");
                 scan.set(i+2,"");
                 i+=2;
                 }
